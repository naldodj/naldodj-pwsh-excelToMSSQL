...
function DiffMegaInsertInToSQL {
    ...
    ForEach ($SRDItem in $SRDItems)
    {
        ...
        [string]$SQLQuery4Output=Invoke-Sqlcmd -query $SQLQuery4 -ServerInstance $SQLInstance -Username $SQLUsername -Password $SQLPassword -TrustServerCertificate -OutputSqlErrors $true
    }
